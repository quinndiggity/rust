...
pub struct RegionInferenceContext<'tcx> {
    ...
    constraints_scc: Rc<Sccs<RegionVid, ConstraintSccIndex>>,
...
impl<'tcx> RegionInferenceContext<'tcx> {
    ...
    pub(crate) fn new(
        ...
        let constraints_scc = Rc::new(constraints.compute_sccs(num_region_variables));

        let scc_values = RegionValues::new(elements, constraints_scc.num_sccs());
    ...
    crate fn region_contains(&self, r: impl ToRegionVid, p: impl ToElementIndex) -> bool {
        let scc = self.constraints_scc.scc(r.to_region_vid());
    ...
    pub(super) fn add_live_element(
        ...
        if self.liveness_constraints.add_element(v, elem) {
            ...
            let scc = self.constraints_scc.scc(v);
    ...
    fn propagate_constraints_scc(&mut self, scc_a: ConstraintSccIndex) {
        ...
        for &scc_b in constraints_scc.successors(scc_a) {
            debug!(
                "propagate_constraints_scc: scc_a = {:?} scc_b = {:?}",
                scc_a, scc_b
            );
        ...
        debug!(
            "propagate_constraints_scc: scc_a = {:?} has value {:?}",
            scc_a,
    ...
    pub fn to_error_region(&self, r: RegionVid) -> Option<ty::Region<'tcx>> {
        if self.universal_regions.is_universal_region(r) {
            ...
            let r_scc = self.constraints_scc.scc(r);
    ...
    fn eval_outlives(
        ...
        let sub_region_scc = self.constraints_scc.scc(sub_region);
        let sup_region_scc = self.constraints_scc.scc(sup_region);
    ...
    fn check_universal_region<'gcx>(
        ...
        let longer_fr_scc = self.constraints_scc.scc(longer_fr);
